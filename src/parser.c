...
void *fh_parse_error(struct fh_parser *p, struct fh_src_loc loc, char *fmt, ...) {
    ...
    fh_set_error(p->prog, "%s:%d:%d: %s",
                 fh_get_ast_file_name(p->ast, loc.file_id), loc.line, loc.col, str);
    return NULL;
}
...
static struct fh_p_expr *new_expr(struct fh_parser *p, struct fh_src_loc loc,
                                  enum fh_expr_type type, size_t extra_size) {
...
static struct fh_p_stmt *new_stmt(struct fh_parser *p, struct fh_src_loc loc,
                                  enum fh_stmt_type type, size_t extra_size) {
...
static int resolve_expr_stack(struct fh_parser *p, struct fh_src_loc loc,
                              struct fh_p_expr **opns, struct opr_info **oprs, int32_t stop_prec) {
    while (1) {
        //printf("********** STACKS ********************************\n");
...
static struct fh_p_expr *parse_expr(struct fh_parser *p, bool consume_stop, char *stop_chars) {
    ...
    while (1) {
        ...
        if (tok_is_punct(&tok, '.')) {
            ...
            index->data.str = fh_buf_add_string(&p->ast->string_pool,
...
static struct fh_p_stmt *parse_stmt_elif(struct fh_parser *p) {
    struct fh_token tok;
    if (get_token(p, &tok) < 0)
        return NULL;
    if (!tok_is_punct(&tok, '('))
    ...
    struct fh_p_stmt *stmt = new_stmt(p, tok.loc, STMT_ELIF, 0);
    if (!stmt)
        return NULL;
    stmt->data.stmt_elif.test = NULL;
    stmt->data.stmt_elif.stmt = NULL;

    stmt->data.stmt_elif.test = parse_expr(p, true, ")");
    if (!stmt->data.stmt_elif.test)
        goto err;

    stmt->data.stmt_elif.stmt = parse_stmt(p);
    if (!stmt->data.stmt_elif.stmt)
        goto err;
...
static struct fh_p_stmt *parse_stmt_if(struct fh_parser *p) {
    ...
    stmt->data.stmt_if.num_elif_stmts = 0;
    ...
    if (tok_is_keyword(&tok, KW_ELIF)) {
        while (tok_is_keyword(&tok, KW_ELIF)) {
            if (stmt->data.stmt_if.num_elif_stmts >= FH_MAX_NESTED_ELIF) {
                fh_parse_error(p, tok.loc, "Cannot parse more than 32 elifs statements");
                goto err;
            }

            stmt->data.stmt_if.elif_stmt[stmt->data.stmt_if.num_elif_stmts] = parse_stmt_elif(p);
            if (!stmt->data.stmt_if.elif_stmt[stmt->data.stmt_if.num_elif_stmts])
                goto err;

            //printf("No. of elif: %zu\n",stmt->data.stmt_if.num_elif_stmts);
    ...
    } else if (tok_is_keyword(&tok, KW_ELSE)) {
        stmt->data.stmt_if.false_stmt = parse_stmt(p);
        if (!stmt->data.stmt_if.false_stmt)
            goto err;
    } else {
...
static struct fh_p_stmt *parse_stmt_repeat(struct fh_parser *p) {
    struct fh_p_stmt *stmt = new_stmt(p, p->last_loc, STMT_REPEAT, 0);
    if (!stmt)
        return NULL;
    ...
    if (!tok_is_keyword(&tok, KW_UNTIL))
        return fh_parse_error_expected(p, tok.loc, "'until'");
...
static struct fh_p_stmt *parse_stmt_for(struct fh_parser *p) {
    struct fh_token tok;
    if (get_token(p, &tok) < 0)
        return NULL;
    if (!tok_is_punct(&tok, '('))
    ...
    struct fh_p_stmt *stmt = new_stmt(p, tok.loc, STMT_FOR, 0);
    if (!stmt)
        return NULL;
    stmt->data.stmt_for.test = NULL;
    stmt->data.stmt_for.init = NULL;
    stmt->data.stmt_for.increment = NULL;
    stmt->data.stmt_for.stmt = NULL;

    stmt->data.stmt_for.init = parse_stmt(p);
    if (!stmt->data.stmt_for.init)
        goto err;
    stmt->data.stmt_for.test = parse_expr(p, true, ";");
    if (!stmt->data.stmt_for.test)
        goto err;
    stmt->data.stmt_for.increment = parse_expr(p, true, ")");
    if (!stmt->data.stmt_for.increment)
        goto err;

    stmt->data.stmt_for.stmt = parse_stmt(p);
...
static struct fh_p_stmt_block *parse_block_after_first_token(
    struct fh_parser *p, struct fh_token *tok,
    struct fh_p_stmt_block *block, bool allow_semicolon) {
    vec_init(&block->stmt_vector);

    if (allow_semicolon) {
        if (!tok_is_punct(tok, ';') && !tok_is_punct(tok, '{')) {
            fh_parse_error_expected(p, tok->loc, "';' or '{'");
            goto err;
        }
        if (tok_is_punct(tok, ';')) {
            struct fh_p_stmt *stmt = new_stmt(p, tok->loc, STMT_EMPTY, 0);
            if (!stmt) goto err;
            vec_push(&block->stmt_vector, stmt);
            return block;
        }
    } else {
        if (!tok_is_punct(tok, '{')) {
...
static struct fh_p_stmt_block *parse_block(struct fh_parser *p, struct fh_token *tok,
                                           struct fh_p_stmt_block *block) {
    if (get_token(p, tok) < 0) return NULL;
    return parse_block_after_first_token(p, tok, block, /*allow_semicolon=*/true);
}

static struct fh_p_expr *parse_func(struct fh_parser *p) {
    struct fh_token tok;

    // by default, we force all functions to have "()"
    bool opt_paranthesis = false;

    if (get_token(p, &tok) < 0)
        return NULL;
    if (!tok_is_punct(&tok, '(')) {
        opt_paranthesis = true;
        unget_token(p, &tok);
    }
    ...
    /*
     * Here we're looking for a docstring
     * then we advance to parse the block '{}'.
     */
    if (tok_is_punct(&tok, ':')) {
        if (get_token(p, &tok) < 0)
            return NULL;
        if (tok.type != TOK_STRING) {
            free(func);
            return fh_parse_error(p, tok.loc, "Expected string after ':'");
        }

        const char *doc = fh_get_ast_string(p->ast, tok.data.str);
        size_t len = strlen(doc) + 1;
        func->data.func.doc_string = malloc(len);
        strcpy(func->data.func.doc_string, doc);
    } else {
        func->data.func.doc_string = NULL;
        if (!opt_paranthesis) {
            unget_token(p, &tok);
        }
    }
