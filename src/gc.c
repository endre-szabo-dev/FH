...
static void debug_obj(const char *prefix, union fh_object *obj) {
    ...
    switch (obj->header.type) {
        ...
        case FH_VAL_ARRAY:
            ...
            break;
        case FH_VAL_MAP:
            printf(" (map of len %d, cap %d)\n", GET_OBJ_MAP(obj)->len, GET_OBJ_MAP(obj)->cap);
            break;
        default:
...
static inline size_t obj_size(union fh_object *obj) {
    switch (obj->header.type) {
        case FH_VAL_STRING:
            return sizeof(struct fh_string) + GET_OBJ_STRING(obj)->size;
        case FH_VAL_CLOSURE:
            return sizeof(struct fh_closure) + sizeof(struct fh_upval *) * GET_OBJ_CLOSURE(obj)->n_upvals;
        case FH_VAL_UPVAL:
            return sizeof(struct fh_upval);
        case FH_VAL_ARRAY:
            return sizeof(struct fh_array) + sizeof(struct fh_value) * GET_OBJ_ARRAY(obj)->cap;
        case FH_VAL_MAP:
            return sizeof(struct fh_map) + sizeof(struct fh_map_entry) * GET_OBJ_MAP(obj)->cap;
        case FH_VAL_FUNC_DEF: {
...
void fh_free_program_objects(struct fh_program *prog) {
    ...
    while (o) {
        ...
        debug_obj("-> FREEING", o);
        fh_free_object(prog, o);
        o = next;
    }
    for (size_t i = 0; i < prog->c_vals.length; i++) {
        struct fh_value *val = prog->c_vals.data[i];
        debug_obj("-> FREEING C_VAL", val);
        free(val);
    }
}

#define MARK_VALUE(gc, v) do { \
if (VAL_IS_OBJECT(v)) MARK_OBJECT((gc), (union fh_object *)((v)->data.obj)); \
} while (0)
#define MARK_OBJECT(gc, o) do { \
union fh_object *_o = (union fh_object *)(o); \
if (!GC_HAS_BIT(_o, GC_BIT_MARK)) \
mark_object((gc), _o); \
} while (0)
...
static void mark_object(struct fh_gc_state *gc, union fh_object *obj) {
    ...
    switch (obj->header.type) {
        ...
        case FH_VAL_C_OBJ:
            return; // treat as leaf (no children)
...
static void mark_func_def_children(struct fh_gc_state *gc, struct fh_func_def *func_def) {
    ...
    for (size_t i = 0; i < func_def->n_consts; i++)
...
static void mark_closure_children(struct fh_gc_state *gc, struct fh_closure *closure) {
    ...
    if (closure->doc_string) {
        MARK_OBJECT(gc, closure->doc_string);
    }
    for (size_t i = 0; i < closure->n_upvals; i++)
...
static void mark_container_children(struct fh_gc_state *gc) {
    while (gc->container_list) {
        ...
        switch (gc->container_list->header.type) {
            ...
            case FH_VAL_C_OBJ: {
                struct fh_c_obj *o = GET_OBJ_C_OBJ(gc->container_list);
                gc->container_list = o->gc_next_container;
                continue;
            }
...
static void mark_roots(struct fh_gc_state *gc, struct fh_program *prog) {
    ...
    for (size_t i = 0; i < prog->pinned_objs.length; i++) {
        MARK_OBJECT(gc, (union fh_object*)prog->pinned_objs.data[i]);
    }

    // mark C values
    debug_log1("***** marking %d C tmp values\n", prog->c_vals.length);
    for (size_t i = 0; i < prog->c_vals.length; i++) {
        MARK_VALUE(gc, (struct fh_value*) prog->c_vals.data[i]);
    }
}
